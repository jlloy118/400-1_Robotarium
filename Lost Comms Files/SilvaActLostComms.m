...
for idx = 1:length(platoon_sizes)
    ...
    tau = ts;  % Time constant for actuator dynamics
    f = zeros(2, N);  % Actuator force for each robot
    mass = 0.09;  % Mass of the robots
    ...
    x_min_boundary = boundaries(1) + 0.14;
    x_max_boundary = boundaries(2) - 0.14;
    y_min_boundary = boundaries(3);
    y_max_boundary = boundaries(4);
    ...
    for i = 1:N
        x_star_initial(1, i) = x_ref(1, 1) + i * desired_spacing(1);
        x_star_initial(2, i) = x_ref(2, 1);
        x_star_initial(3, i) = pi;
    end
    ...
    for t = start_time:iterations
        ...
        dxi = current_dxi(:, :, n);            
            ...
            % Control input (force) for actuator dynamics
            u = HMinusOne(HiP(Kp1, Kp2, front_error), Kv, front_vel_error) ...
                + epsilon * HPlusOne(HiP(Kp1, Kp2, rear_error), Kv, rear_vel_error) ...
                + HZero(KpZero, error_zero, KvZero, dxl(:, reftimestamp), dxi(1:2, n));
            ...
            u = u + integral_error(:,n);
            % Update actuator force with first-order dynamics
            f(:, n) = f(:, n) + (u - f(:, n)) * (ts / tau);

            % Calculate the acceleration from the actuator force
            acceleration_input(:, n) = f(:, n) / mass + disturbance/mass;
            ...
            positions(:, t:iterations, n) = repmat(xi(1:2, n), 1, iterations - t + 1);
        ...
        for i = 1:N
            if method == 3
                dxi(1, i) = dxi(1, i) + acceleration_input(1, i) * ts;
                dxi(2, i) = dxi(2, i) + acceleration_input(2, i) * ts;
            else
        ...
        dxu = si_to_uni_dyn(dxi, actual_positions);
        if safety_on == true
            dxu = uni_barrier_cert(dxu, actual_positions);
        end
        r.set_velocities(1:N, dxu);
        r.step();
    end
    ...
    save("supremumreferrorsilvaactblackout","supremum_reference_error");
    ...
    save("silvaactsupremumspacingblackout","max_spacing_error")
    ...
    maxspacingerrortotalsilvactlostcomms = max(max_spacing_error);
    maxsupremreferrortotalsilvactlostcomms = max(supremum_reference_error);

    save(sprintf("supremum_error_platoon_size_%d_silvact", N), 'maxsupremreferrortotalsilvactlostcomms');
    save(sprintf("spacing_error_platoon_size_%d_silvact", N), 'maxspacingerrortotalsilvactlostcomms');
    hold off;
end
...
for idx = 1:length(platoon_sizes)
    ...
    % Plot the dot
    plot(N, maxsupremreferrortotalsilvactlostcomms, 'ko', 'MarkerFaceColor', 'b');  % Black filled dot

    % Plot the vertical dotted line
    line([N N], [0 maxsupremreferrortotalsilvactlostcomms], 'LineStyle', '--', 'Color', 'b');  % Vertical line
end
...
for idx = 1:length(platoon_sizes)
    ...
    % Plot the dot
    plot(N, maxspacingerrortotalsilvactlostcomms, 'ko', 'MarkerFaceColor', 'k');  % Black filled dot

    % Plot the vertical dotted line
    line([N N], [0 maxspacingerrortotalsilvactlostcomms], 'LineStyle', '--', 'Color', 'k');  % Vertical line
end
