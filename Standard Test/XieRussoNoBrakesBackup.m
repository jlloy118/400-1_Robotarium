...
platoon_sizes = [26];  % Example with 6 robots
%platoon_sizes = [2,4,8,12,15];
...
for idx = 1:length(platoon_sizes)
    ...
    spacing_const = 0.3;          % Desired spacing between robots
    desired_spacing = [spacing_const; 0];  % Spacing in x and y direction
    vconst = 0.12;                % Constant velocity for path traversal
    ...
    si_to_uni_dyn = create_si_to_uni_dynamics('LinearVelocityGain', 1, 'AngularVelocityLimit', pi);
...
for t = start_time:iterations
    ...
    % Add disturbances
    disturbance = disturbance_amplitude * exp(-0.1 * (t - start_time) * ts) * sin((t - start_time) * ts) + disturbance_amplitude;
    ...
    for n = 1:N
        ...
        dr1(:, n, t-1) = k2 * ((xl(1:2, t-1) - xi(1:2, n, t-1)) - (xl(1:2, t-1) - xd(1:2, n, t-1)));
        dr0(:, n, t) = k1 * ((xl(1:2, t) - xi(1:2, n, t)) - (xl(1:2, t) - xd(1:2, n, t)));
        dxi(:, n, t) = k0 * ((xl(1:2, t) - xi(1:2, n, t)) - (xl(1:2, t) - xd(1:2, n, t))); 

        % Add neighbor coupling effects
        if n ~= 1
            dr1(:,n,t-1) = dr1(:,n,t) + k2tau * tanh(kpsi * ((xi(1:2, n-1, t) - xi(1:2, n, t)) - (xd(1:2, n-1, t) - xd(1:2, n, t))));
            dr0(:,n,t-1) = dr0(:,n,t-1) + k1tau * tanh(kpsi * ((xi(1:2, n-1, t) - xi(1:2, n, t)) - (xd(1:2, n-1, t) - xd(1:2, n, t))));
        end
        
        if n ~= N
            dr1(:,n,t) = dr1(:,n,t) + k2tau * tanh(kpsi * ((xi(1:2, n+1, t) - xi(1:2, n, t)) - (xd(1:2, n+1, t) - xd(1:2, n, t))));
            dr0(:,n,t) = dr0(:,n,t) + k1tau * tanh(kpsi * ((xi(1:2, n+1, t) - xi(1:2, n, t)) - (xd(1:2, n+1, t) - xd(1:2, n, t))));
        end

        % Apply the changes immediately to r0 and r1 in the same time step
        r1(:, n, t) = r1(:,n,t-1) + dr1(:, n, t-1) * ts;  % Update r1 using current dr1
        dr0(:,n,t) = dr0(:,n,t) + r1(:,n,t);
        r0(:, n, t) = r0(:,n,t-1) + dr0(:, n, t-1) * ts;  % Update r0 using current dr0
        
        % Update dxi with the current r0 and disturbances
        dxi(:, n, t) = dxi(:, n, t) + r0(:, n, t)+ disturbance + dxl(:, reftimestamp);

        % Add final neighbor coupling effect for dxi
        if n ~= 1
            dxi(:,n,t) = dxi(:,n,t) + k0tau * tanh(kpsi * ((xi(1:2, n-1, t) - xi(1:2, n, t)) - (xd(1:2, n-1, t) - xd(1:2, n, t))));
        end
        if n ~= N
            dxi(:,n,t) = dxi(:,n,t) + k0tau * tanh(kpsi * ((xi(1:2, n+1, t) - xi(1:2, n, t)) - (xd(1:2, n+1, t) - xd(1:2, n, t))));
        end
