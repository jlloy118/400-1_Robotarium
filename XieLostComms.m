...
for idx = 1:length(platoon_sizes)
    ...
    affected_robots = N:-4:1;  % Specify which robots will lose communication
    last_known_positions = zeros(3, N); % To store the last known positions during blackout
...
for t = start_time:iterations
    ...
    if t > 1
        actual_dxi = dxi(:, :, t - 1);    % Use velocities from previous time step
    else
        actual_dxi = zeros(2, N);         % Initialize if t == 1
    end
    ...
    disturbance = disturbance_amplitude * exp(-0.1 * (t - start_time) * ts) * sin((t - start_time) * ts) + 5 * disturbance_amplitude;
        ...
        for n = 1:N
            % Use temporary variables for robot n's accessible positions and velocities
            xi_n = current_poses(:, :, n);    % Accessible positions for robot n
            dxi_n = current_dxi(:, :, n);     % Accessible velocities for robot n
            ...
            % Proceed with your original code using xi_n and dxi_n
            % For example:
        ...
        % Use xi_n and dxi_n instead of xi and dxi
        error_zero(:, n, t) = xd(:, n, t) - xi_n(1:2, n);
        dr1(:, n, t) = k2 * ((xl(1:2, t) - xi_n(1:2, n)) - (xl(1:2, t) - xd(1:2, n, t)));
        dr0(:, n, t) = k1 * ((xl(1:2, t) - xi_n(1:2, n)) - (xl(1:2, t) - xd(1:2, n, t)));
        dxi(:, n, t) = k0 * ((xl(1:2, t) - xi_n(1:2, n)) - (xl(1:2, t) - xd(1:2, n, t)));

        % Neighbor coupling
        neighbours = find(comm_matrix(n,:) ~= 0);
        for j = neighbours
            dr1(:, n, t) = dr1(:, n, t) + k2tau * tanh(kpsi * ((xi_n(1:2, j) - xi_n(1:2, n)) - (xd(1:2, j, t) - xd(1:2, n, t))));
            dr0(:, n, t) = dr0(:, n, t) + k1tau * tanh(kpsi * ((xi_n(1:2, j) - xi_n(1:2, n)) - (xd(1:2, j, t) - xd(1:2, n, t))));
        end
        ...
        for j = neighbours
            dxi(:, n, t) = dxi(:, n, t) + k0tau * tanh(kpsi * ((xi_n(1:2, j) - xi_n(1:2, n)) - (xd(1:2, j, t) - xd(1:2, n, t))));
        end
        ...
        if velocity_magnitude > 0.2
            ...
            set(plot_handles_xi(n), 'XData', actual_positions(1, n), 'YData', actual_positions(2, n));
            ...
            set(text_handles_xi(n), 'Position', [actual_positions(1, n), actual_positions(2, n)], 'String', num2str(n));
        ...
        for i = 2:N
            current_spacing = norm(actual_positions(1:2, i) - actual_positions(1:2, i-1));
            spacing_error(t, i-1) = abs(current_spacing - spacing_const);
        end

        %% Update Robot Velocities and Step Simulation %%
        dxu = si_to_uni_dyn(dxi(:,:,t), actual_positions);  % Convert to unicycle dynamics
        if safety_on
            dxu = uni_barrier_cert(dxu, actual_positions);  % Apply barrier certificate if safety is enabled
        end
        r.set_velocities(1:N, dxu);  % Set velocities for robots
        r.step();  % Step the simulation
    end
    ...
    save("supremerrorxieblackout","supremum_reference_error");
    ...
    save("spacingerrorxieblackout","max_spacing_error");
    ...
    maxspacingerrortotalxielostcomms = max(max_spacing_error);
    maxsupremreferrortotalxielostcomms = max(supremum_reference_error);
    final_poses = r.get_poses();
    r.debug();
    save(sprintf("supremum_error_platoon_size_%d_xie", N), 'maxsupremreferrortotalxielostcomms');
    save(sprintf("spacing_error_platoon_size_%d_xie", N), 'maxspacingerrortotalxielostcomms');
...
for idx = 1:length(platoon_sizes)
    ...
    plot(N, maxsupremreferrortotalxielostcomms, 'ko', 'MarkerFaceColor', 'b');  % Plot dot
    line([N N], [0 maxsupremreferrortotalxielostcomms], 'LineStyle', '--', 'Color', 'b');  % Plot vertical line
end
...
for idx = 1:length(platoon_sizes)
    ...
    plot(N, maxspacingerrortotalxielostcomms, 'ko', 'MarkerFaceColor', 'k');  % Plot dot
    line([N N], [0 maxspacingerrortotalxielostcomms], 'LineStyle', '--', 'Color', 'k');  % Plot vertical line
end
